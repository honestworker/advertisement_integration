...
class WPAdIntgr_Page {
    ...
    private function popup_page_html( $selector, $redirect = 1 ) {
        $page_html = '';
        ...
        if ( isset( $_GET['action'] ) && isset( $_GET['selector'] ) ) {
            if ( $_GET['selector'] != '' ) {
                ...
                if ( $_GET['action'] == 'exit' ) {
                    ...
                    if ( $selector['selector_type'] != '' ) {
                        if ( $selector['leave_exit_check'] == 'on' || isset($_GET['popup']) ) {
                    ...
                    if ( $selector['selector_type'] == 'popup' ) {
                        if ( $selector['popup_exit_check'] == 'on' ) {
                            ...
                            $page_html .= $this->popup_page_html($selector, 0);
                        }
                    }
                }
            }
            ...
            if ( isset( $_GET['selector'] ) ) {
                if ( isset( $_GET['zipcode'] ) ) {
                    if ( $_GET['selector'] != '' ) {
                        $selector = $selectors[$_GET['selector']];
                        $page_html .= $this->popup_page_html($selector);
    ...
    public function include_popup_redirect_code() {
        if ( isset($_POST['adintgr-selector']) ) {
            ...
            <script type="text/javascript">
            jQuery(function($) {
			    setTimeout(() => window.open('<?php echo $selector['popup_url'];?>', '_blank', 'width=' + screen.width + ',height=' + screen.height + ','), 0);
            });
            </script>
        <?php
        }
        ...
        jQuery( function( $ ) {
            document.body.addEventListener('mouseleave', function(e) {
                if (e.pageY - document.body.scrollTop < 0) {
                    ...
                    ?>
                        setTimeout(() => window.open('<?php echo get_permalink($this->pages_id[$_POST['adintgr-selector']]);?>?action=leave&selector=<?php echo $_POST['adintgr-selector'];?>', '_blank', 'width=' + screen.width + ',height=' + screen.height + ','), 0);
                    <?php
                    }
            ?>  }
            });
        ...
        jQuery( function( $ ) {
            document.body.addEventListener('mouseleave', function(e) {
                if (e.pageY - document.body.scrollTop < 0) {
                    ...
                    if ( $selector['popup_exit_type'] == '' ) {
                    ?>
                        setTimeout(() => window.open('<?php echo $selector['popup_exit_url'];?>', '_blank', 'width=' + screen.width + ',height=' + screen.height + ','), 0);
                    <?php
                    } else if ($selector['popup_exit_type'] == 'mediaalpha') {
                    ?>
                        setTimeout(() => window.open('<?php echo get_permalink($this->pages_id[$_GET['selector']]);?>?action=popup&selector=<?php echo $_GET['selector'];?>', '_blank', 'width=' + screen.width + ',height=' + screen.height + ','), 0);
                    <?php
                    }
            ?>  }
            });
